...
-   You want your module to be usable in node `type: module` mode _AND_ you have some `export default` (if you don't have export default it will work just fine).
...
const str = fs.readFileSync(path.join(__dirname, "..", "package.json")).toString("utf8");
```
...
const str = fs.readFileSync(path.join(getProjectRoot(), "package.json")).toString("utf8");
```
